...
seasonlength=mnthlength[m[0]-1]+mnthlength[m[1]-1]+mnthlength[m[2]-1]
#Load daily temperature data
t2mmin=np.empty([seasonlength,361,576]);
t2mmax=np.empty([seasonlength,361,576]);

counter=0
for n in range(3):
	for d in range(mnthlength[m[n]-1]):
		statDfile = Dataset('/discover/nobackup/projects/gmao/merra2/data/products/MERRA2_400' + '/Y' + str(y[n]) + '/M' + str(m[n]).zfill(2) + '/MERRA2_400.statD_2d_slv_Nx.' + str(y[n]) + str(m[n]).zfill(2) + str(d+1).zfill(2) + '.nc4',mode='r')
		t2mmin[counter,:,:]=statDfile.variables['T2MMIN'][:,:,:]
		t2mmax[counter,:,:]=statDfile.variables['T2MMAX'][:,:,:]
		counter=counter+1
...
if not exists(outfile):
	...
	ncfile.RangeEndingDate= str(y[2]) + '-' + str(m[2]).zfill(2) + '-' + str(calendar.monthrange(yr,m[2])[1])
...
def Tpcnt(minmax,pctl):
	...
	pctlthresh=np.concatenate((pctlyr[int(doyindex1[0]):int(doyindex2[0]),:,:], pctlyr[int(doyindex1[1]):int(doyindex2[1]),:,:], pctlyr[int(doyindex1[2]):int(doyindex2[2]),:,:]),axis=0)
	if minmax=='max':
	...
	Tvar=(np.nansum(t2m,axis=0)/seasonlength)*100
	return(Tvar)
