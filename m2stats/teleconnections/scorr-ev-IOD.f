      parameter (nx=120,ny=60,mode=4,ntel=4,nt=1419)
      dimension evref(nx,ny),evref1(ntel,nx,ny),ev(nx,ny),pc(mode)

      open (11,file='evIOD-ANN19612021ref.d',form='unformatted',
     & access='direct',recl=nx*ny,status='old')
      open (12,file='pc-IOD-DJF18702021.d',form='unformatted',
     & access='direct',recl=mode,status='old')
      open (13,file='ev-IOD-DJF18702021.d',form='unformatted',
     & access='direct',recl=nx*ny,status='old')

      open (14,file='pc-IOD-DJF18702021rv.d',form='unformatted',
     & access='direct',recl=1,status='unknown')
      open (15,file='ev-IOD-DJF18702021rv.d',form='unformatted',
     & access='direct',recl=nx*ny,status='unknown')

      itel=2 ! IOD from the second mode
      cormax=0.0
      dmiss=-9999.
